-- Function: squeesar.agrupar_squeesar_poligono()

-- DROP FUNCTION squeesar.agrupar_squeesar_poligono();

CREATE OR REPLACE FUNCTION squeesar.agrupar_squeesar_poligono(integer)
  RETURNS integer AS
$BODY$

DECLARE

	nNumPoligono$	ALIAS FOR $1;
	/*--------------------------------- */
...
BEGIN
	...
	sQuery_Fecha$ := 'SELECT 
		...
		WHERE
			poligono_squeesar.id_poligono = ' || nNumPoligono$ || '
		GROUP BY 
			poligono_squeesar.id_poligono,
			' || sNombreTabla$ || '_fecha.direccion,
			' || sNombreTabla$ || '_fecha.fecha
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION squeesar.agrupar_squeesar_poligono(integer)
  OWNER TO postgres;
